...
export function AudioUploadForm({ userId, onSuccess }: AudioUploadFormProps) {
  ...
  const validateFile = (
    ...
    if (!isValidFileType(file)) {
      return {
        valid: false,
        errorMessage:
          "Invalid file type. Please upload MP3, WAV, or M4A files.",
      };
    }
      ...
      >
        ...
        {selectedFile ? (
          ...
          <div className="space-y-2">
            <Music className="mx-auto size-10 text-muted-foreground" />
            <div>
              <p className="text-sm font-medium">
                Drop audio file here or click to browse
              </p>
              <p className="text-xs text-muted-foreground">
                Supports MP3, WAV, M4A files • Max size: {maxFileSizeDisplay}
              </p>
            </div>
          </div>
        )}
