...
import { trackTextUsageEvent } from "@/lib/monitoring/sentry";

// Constants for magic numbers
const TEXT_USAGE_REFRESH_INTERVAL = 60000; // 1 minute
const TEXT_USAGE_WARNING_THRESHOLD = 75; // %
const TEXT_USAGE_CRITICAL_THRESHOLD = 90; // %

/**
 * TextUsageIndicator Component
 * Circular indicator showing text chat usage limit in the navbar
 * with popover for details and link to usage page
 */
export function TextUsageIndicator() {
  const {
    data: usage,
    isLoading,
    error,
  } = useUserUsage({
    refetchInterval: TEXT_USAGE_REFRESH_INTERVAL,
  });
  ...
  const isUnlimited = text?.messages_limit === -1;
  const percentage = isUnlimited ? 0 : (text?.percentage ?? 0);
  ...
  const colors = useMemo(() => {
    ...
    if (percentage >= TEXT_USAGE_CRITICAL_THRESHOLD) {
      return {
    ...
    if (percentage >= TEXT_USAGE_WARNING_THRESHOLD) {
      return {
    ...
    // Default: blue colors for text chat
    return {
  ...
  useEffect(() => {
    ...
    if (
      percentage >= TEXT_USAGE_WARNING_THRESHOLD &&
      percentage < TEXT_USAGE_CRITICAL_THRESHOLD &&
  ...
  }, [usage, percentage, messagesUsed, messagesLimit, isUnlimited]);

  // Error fallback - show inactive indicator
  if (error) {
    console.error("[TextUsageIndicator] Failed to load usage:", error);
    return (
  ...
  const ariaLabel = isUnlimited
    ? "Text chat usage: Unlimited"
    : `Text chat usage: ${percentage.toFixed(0)}% of monthly limit used`;
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {/* Message icon in center - always black */}
          <MessageSquare className="size-4 text-slate-900" aria-hidden="true" />
        </button>
      </PopoverTrigger>
        ...
        <div className="p-4">
          ...
          <div className="mb-4 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Monthly message limit</p>
            </div>
          </div>
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div
                ...
                aria-valuemin={0}
                aria-valuemax={100}
                aria-label="Text chat usage progress"
              >
                <div
                  className={cn(
              ...
              <div className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                ...
                <span
                  className={cn(
                    ...
                    messagesRemaining < 50 ? "text-red-600" : "text-slate-900",
                  )}
                >
                  {formatMessages(messagesRemaining)}
                </span>
              </div>
            )}
          ...
          {/* Reset date */}
          {usage.text?.reset_date && (
            <p className="mb-3 text-center text-xs text-slate-500">
              Resets{" "}
              {new Date(usage.text.reset_date).toLocaleDateString(undefined, {
                month: "short",
                day: "numeric",
              })}
            </p>
