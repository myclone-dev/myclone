...
async def upload_avatar_background(user_id: str, google_id: str, picture_url: str):
    """
    ...
    try:
        ...
        capture_exception_with_context(
            e,
            extra={"user_id": user_id, "google_id": google_id},
            tags={
                "component": "google_oauth",
                "operation": "upload_avatar_background",
                "severity": "medium",
                "user_facing": "false",
            },
        )
...
async def send_onboarding_email_background(email: str, fullname: str):
    ...
    try:
        ...
        capture_exception_with_context(
            ...
            tags={
                "component": "google_oauth",
                "operation": "send_onboarding_email_background",
                "severity": "medium",
                "user_facing": "false",
            },
        )
...
async def google_login():
    ...
    auth_url = GoogleOAuthService.get_authorization_url(
        state=state, redirect_uri=settings.google_redirect_uri
    )

    logger.info(f"Initiating Google OAuth for state: {state}")
...
async def google_callback(
    ...
    try:
        ...
        token_response = await GoogleOAuthService.exchange_code_for_token(
        ...
        # Use fullname from Google, or construct from given_name + family_name
        if not fullname:
            fullname = f"{given_name} {family_name}".strip() or email.split("@")[0]

        # Check if user exists by Google platform ID
        user = await UserRepository.get_user_by_platform_id(session, "google", google_id)
        ...
        if not user:
            ...
            if user:
                # Existing user - link Google auth to their account (same pattern as LinkedIn)
                # Note: We don't update google_id on user table since we use auth_details table
                # with platform/platform_user_id for multi-provider support
                ...
                # New user - create account and subscription in a single transaction
                ...
                try:
                    user = await UserRepository.create_user(
                        ...
                        email=email,
                        fullname=fullname,
                        avatar=picture,  # Start with Google URL, will be updated by background task
                    )

                    # Create default free tier subscription for new Google user
                    ...
                    logger.info(f"Created free tier subscription for Google user: {email}")

                    should_upload_avatar = bool(picture)
                    is_new_user = True  # Flag to send onboarding email
                except Exception as e:
                    logger.error(f"Failed to create user or subscription: {e}")
                    ...
                    capture_exception_with_context(
                        ...
                        tags={
                            "component": "google_oauth",
                            "operation": "create_user_with_subscription",
                            "severity": "high",
                            "user_facing": "true",
                        },
                    )
        ...
        # Create JWT token (reuse LinkedIn's JWT creation - same format)
        ...
        logger.info(f"Successfully authenticated user via Google: {user.email}")
        return response
